		...
		<div class="row">
			...
			<div class="col-sm-4">
				<div class="pull-right">
					<a href="<?=base_url('teknikkomputer/add')?>" class="btn bg-purple btn-flat btn-sm"><i class="fa fa-plus"></i> Tambah Bank Soal</a>
					<button type="button" onclick="reload_ajax()" class="btn btn-flat btn-sm bg-maroon"><i class="fa fa-refresh"></i> Reload</button>
				</div>
			</div>
		</div>
    </div>
	<?=form_open('teknikkomputer/delete', array('id'=>'bulk'))?>
    <div class="table-responsive px-4 pb-3" style="border: 0">
        <table id="teknikkomputer" class="w-100 table table-striped table-bordered table-hover">
        <thead>
            <tr>
...
<script src="<?=base_url()?>assets/dist/js/app/teknikkomputer/data.js"></script>
...
$(document).ready(function(){
	$('#matkul_filter').on('change', function(){
		...
		let src = '<?=base_url()?>teknikkomputer/data';
		let url;
